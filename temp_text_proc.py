...
tp.wordnet_lemmatize_string(input_string = "The striped bats are hanging on their feet for best")
...
def get_text_count_features(input_string, delimiter = ''):
...
tt = list(train_df[config.config_complaints_narrative_column])
tt2 = tt[0:5000]

temp = clean_text(tt2)


raw_word_counts = get_word_counts(temp)
raw_word_counts_df = pd.DataFrame(raw_word_counts.items()).\
sort_values(by = 1, ascending = False)



raw_word_counts_df = pd.DataFrame({'word' : raw_word_counts_df.columns, 'freq' : raw_word_counts_df.values})



#44
temp2 = [replace_contractions(s, config.config_contraction_dict) for s in temp] 
temp3 = [remove_non_alpha(s) for s in temp2]   



 

tt_counts = get_word_counts(temp)
